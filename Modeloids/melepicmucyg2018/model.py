...
class Model(object):
    ...
    def get_adversarial_loss(self, discriminator, name = "D", scope_to_reuse=None):
        ...
        if (self.config['gan']['type'] == 'wgan'
            ...
            if self.config['gan']['type'] == 'wgan-gp': #do not work with dynamic rnn
                ...
                if scope_to_reuse is None:
                    D_inter = discriminator(inter, self.G.tensor_len, self.config, name=name,
                                            reuse=True)
                else:
    ...
    def get_reconstruction_loss(self, scope_to_reuse=None):
        """Return the rescontruction losses."""
        if self.config['cycle']['type'] == 'mse':
            rec_loss = tf.losses.mean_squared_error(self.G.tensor_in, self.G_inv.tensor_out)

        if self.config['cycle']['type'] == 'cosine':
            rec_loss = tf.losses.cosine_distance(self.G.tensor_in, self.G_inv.tensor_out)

        if self.config['cycle']['type'] == 'hinge':
            rec_loss = tf.losses.hinge_loss(self.G.tensor_in, self.G_inv.tensor_out)
    ...
    def stats(self):
        ...
        n_params["global"] = get_num_parameter(tf.trainable_variables(
            self.scope.name))
        for d in datasets:
            n_params[d+"_dis"] = get_num_parameter(self.nets[d+"_dis"].vars)
        for pair in permutations(datasets,2):
            n_params[pair[0]+"_to_"+pair[1]+"_gen"] = get_num_parameter(self.nets[pair[0]+"_to_"+pair[1]].vars)
